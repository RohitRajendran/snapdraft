 ...
 */
export function axisSnap(raw: Point, origin: Point, gridIncrement = GRID_SNAP_FT): Point | null {
  ...
  if (dy <= SNAP_RADIUS_FT && dy < dx) {
    ...
    return { x: snapToGrid(raw.x, gridIncrement), y: origin.y };
  }
  if (dx <= SNAP_RADIUS_FT && dx < dy) {
    // Close to vertical — lock X to origin, grid-snap Y
    return { x: origin.x, y: snapToGrid(raw.y, gridIncrement) };
  }
  return null;
}
...
export function useSnap(elements: Element[], chainOrigin?: Point | null) {
  const snap = useCallback(
    (rawPoint: Point, gridIncrement = GRID_SNAP_FT): Point => {
      ...
      // 2: nearest point on any wall centerline
      ...
      if (chainOrigin) {
        const axisPt = axisSnap(rawPoint, chainOrigin, gridIncrement);
        if (axisPt) return axisPt;
      }

      // 4: grid snap
      return snapPointToGrid(rawPoint, gridIncrement);
    },
    [elements, chainOrigin],
  );

  const snapWithInfo = useCallback(
    (rawPoint: Point, gridIncrement = GRID_SNAP_FT): SnapResult => {
      // 1: existing wall endpoint
      const endpoints = collectEndpoints(elements);
      const nearestEndpt = findNearestEndpoint(rawPoint, endpoints);
      if (nearestEndpt)
      ...
      // 4: grid snap
      return {
        point: snapPointToGrid(rawPoint, gridIncrement),
        snappedToEndpoint: false,
        snappedToSegment: false,
        snappedToAxis: false,
      };
